...
var Types = {
     ...
     * ````
     *
     * @param {Type} type TypeCheck if value is not null or undefined
     * @returns {Boolean|String} ok True if the check is successfull, or an error string
     */
    Maybe: function(type) {
        var typeCheck = Types.wrap(type);
        ...
        return function Maybe(value) {
            ...
            if (!_isError(err)) return true;

            return err || _formatError(null, 'Maybe[' + typeCheck.name + ']', typeCheck.name, value);
        };
    },
     ...
     *
     * ```
     * @param {String} name Optional name for this DuckType to identify it with
     * @param {Object} typedef Plain object of name => Type pairs
     * @returns {Boolean|String} ok True if the check is successfull, or an error string
     */
